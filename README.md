...
지금은 매번 동일한 쿼리로 테스트 -> 좀 더 다양한 조건에서 테스트를 하려면 docker 컨태이너를 내렸다 다시 실행하며 데이터베이스 내용을 바꿔가며 테스트 가능
후에 실행될 쿼리들도 동적으로 랜덤하게 바꿔가며 테스트 하게 하면 좀 더 의미있는 데이터 추출 가능

현 상황으로는 도커 컨테이너를 반복적으로 재실행하며 비슷한 경향을 보임을 확인하여 현 구조 유지
